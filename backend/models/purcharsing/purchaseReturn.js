...
purchaseReturnSchema.pre('save', function(next) {
  ...
  this.remainingQty = this.purchaseQty - this.usedQty - this.returnQuantity;
  
  // Auto-calculate return amount if not provided
  if (this.returnAmount === 0 && this.amount > 0 && this.purchaseQty > 0) {
    this.returnAmount = (this.amount / this.purchaseQty) * this.returnQuantity;
  }
...
purchaseReturnSchema.index({ productName: 1 });
...
purchaseReturnSchema.virtual('unitPrice').get(function() {
  return this.purchaseQty > 0 ? this.amount / this.purchaseQty : 0;
});
  ...
  const result = await this.aggregate([
    ...
    { 
      $group: { 
        _id: '$status', 
        totalReturns: { $sum: 1 },
        totalQuantity: { $sum: '$returnQuantity' },

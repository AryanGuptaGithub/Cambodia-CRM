...
const findProductStockInHand = async (productName, requiredQty) => {
  try {
    let searchName = normalizeProductName(productName);
    console.log(`After normalizeProductName: "${searchName}"`);

    const fixedName = productNameFixMap[searchName] || searchName;
    console.log(`After fix map lookup: "${fixedName}"`);

    // Escape special regex chars
    const escaped = fixedName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    console.log(`Escaped for regex: "${escaped}"`);
    ...
    console.log(`Exact match found: ${stockItems.length} items`);
    ...
    if (stockItems.length === 0) {
      ...
      console.log(`Partial match found: ${stockItems.length} items`);
      if (stockItems.length > 0) {
        console.log("Matched product names from DB:");
        stockItems.forEach((item, idx) => {
          console.log(
            `  [${idx + 1}] "${item.productName}" (currentStock: ${
              item.currentStock
            })`
          );
        });
    ...
    console.log(`Total available stock calculated: ${available}`);

    if (available < requiredQty) {
      console.log(
        `❌ INSUFFICIENT STOCK: Required ${requiredQty}, Available ${available}`
      );
    ...
    console.log(
      `✅ SUFFICIENT STOCK: Available ${available} >= Required ${requiredQty}`
    );
    ...
    console.error(
      `🚨 ERROR in stock check for "${productName}":`,
      error.message
    );
...
// ====================== STOCK CONSUMPTION (FIFO - ONLY ON SUCCESS) ======================
...
const consumeStockFromHand = async (productName, requiredQty, session) => {
  try {
    ...
    for (const item of stockItems) {
      ...
      const take = Math.min(item.currentStock || 0, remaining);
      if (take > 0) {
    ...
    if (remaining > 0) {
      throw new Error(`Could not deduct full quantity for ${productName}`);
    }
...
// ====================== BATCH PROCESSING WITH STOCK DEDUCTION ======================

const processImportBatch = async (batch, sessionId) => {
  const progress = importProgressMap.get(sessionId);
  if (!progress)
    return { success: 0, failed: batch.length, failedInvoices: [] };

  const results = { success: 0, failed: 0, failedInvoices: [] };
      ...
      for (const saleData of batch) {
        try {
          ...
          for (const p of saleData.products) {
            ...
            // Deduct stock only for sold quantity (not bonus)
            if (salesQty > 0) {
          ...
          results.failedInvoices.push({
            ...
            error: err.message,
            type: err.message.includes("stock")
              ? "insufficient_stock"
              : "import_error",
  ...
  progress.successful += results.success;
  progress.failed += results.failed;
  progress.errors.push(...results.failedInvoices);
...
const processImportAsync = async (sessionId, validInvoices, batchSize = 10) => {
  const progress = importProgressMap.get(sessionId);
  if (!progress) return;
  ...
  validInvoices.forEach((sale, idx) => {
    (sale.products || []).forEach((p) => {
      const qty = parseFloat(p.salesQty) || 0;
      if (qty <= 0) return;
      const key = normalizeProductName(p.productName);
      if (!stockMap.has(key))
        stockMap.set(key, { name: p.productName, totalQty: 0, affected: [] });
      const entry = stockMap.get(key);
      entry.totalQty += qty;
      entry.affected.push({
        sale,
        row: idx + 2,
        invoice: sale.invoiceNumber || `Row-${idx + 2}`,
      });
    });
  });

  const insufficientProducts = [];
  for (const [_, data] of stockMap) {
    const check = await findProductStockInHand(data.name, data.totalQty);
    if (check.insufficient) {
      insufficientProducts.push({ productName: data.name, ...check });
      data.affected.forEach(({ invoice, row }) => {
        stockFailed.push({
          row,
  ...
  progress.insufficientStockProducts = insufficientProducts;
  progress.errors = stockFailed;

  // Filter good invoices
  const goodInvoices = validInvoices.filter((sale) =>
    (sale.products || []).every((p) => {
      const qty = parseFloat(p.salesQty) || 0;
      if (qty <= 0) return true;
      const key = normalizeProductName(p.productName);
      return !insufficientProducts.some(
        (issue) => normalizeProductName(issue.productName) === key
      );
    })
  );
  ...
  // Process in batches
  const batches = [];
  for (let i = 0; i < goodInvoices.length; i += batchSize) {
    batches.push(goodInvoices.slice(i, i + batchSize));
  }

  for (const batch of batches) {
    await processImportBatch(batch, sessionId);
  }

  updateImportProgress(sessionId, { completed: true });
};
...
router.post("/sales/import", async (req, res) => {
  ...
  try {
    ...
    processImportAsync(sessionId, dataToImport, 10);

    res.json({
      success: true,
      message:
        "Import started – stock checked, valid invoices will be imported with stock deduction",
      sessionId,
...
router.get("/sales/import/progress/:sessionId", (req, res) => {
  ...
  res.json({
    ...
    progress: {
      ...
      stockIssues: progress.insufficientStockProducts?.length || 0,
...
router.get("/sales/import/failed/:sessionId", (req, res) => {
  ...
  const failedInvoices = [
    ...progress.errors,
    ...progress.insufficientStockProducts.flatMap(
      (issue) =>

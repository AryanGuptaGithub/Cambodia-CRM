...
const consumeStockFromHand = async (productName, requiredQty, session = null) => {
  try {
    const normalized = normalizeProductName(productName);
    const fixedName = productNameFixMap[normalized] || productName.trim();
    const escaped = fixedName.replace(/[.*?^${}()|[\]\\]/g, "\\$&");
    ...
    if (stockItems.length === 0) {
      let allItems;
      if (session) {
        allItems = await ReportInHand.find({}).session(session);
      } else {
        allItems = await ReportInHand.find({});
      }
    ...
    if (remaining > 0) {
      throw new Error(
        `Could only deduct ${requiredQty - remaining} of ${requiredQty} for ${productName}. Insufficient stock.`
      );
    }
...
const findMRStaff = async (mrName, mrId) => {
  ...
  if (mrName && mrName.trim() && mrName.trim().toLowerCase() !== "no mr name provided") {
...
const addCashToMR = async (saleData) => {
  ...
  // Only add cash for Cash payment status, not for Credit
  if (paymentStatus !== "Cash" || paidAmount <= 0) {
  ...
  if (!mrStaff) {
    console.warn(`No valid Staff record for MR "${mrName}". Skipping cash update.`);
...
const removeCashFromMR = async (saleData) => {
  ...
  // Only remove cash for Cash payment status
  if (paymentStatus !== "Cash" || paidAmount <= 0) {
    return { success: false };
  }
  ...
  if (!mrStaff) {
    console.warn(`No valid Staff record for MR "${mrName}". Skipping cash removal.`);
    return { success: false, skipped: true };
  }
...
// FIXED: Updated mapPaymentStatus to handle importStatus logic
const mapPaymentStatus = (status, importStatus = null) => {
  if (!status) {
    // If importStatus is provided, use it to determine payment status
    if (importStatus && importStatus.toLowerCase() === "pending") {
      return "Credit";
    }
    if (importStatus && importStatus.toLowerCase() === "paid") {
      return "Cash";
    }
    return "Credit";
  }
  ...
  // Special handling: if status is "Paid", check importStatus
  if (s === "paid") {
    if (importStatus && importStatus.toLowerCase() === "pending") {
      return "Credit"; // When importStatus is pending, paymentStatus should be Credit
    }
    return "Cash"; // Otherwise, Paid becomes Cash
  }
...
const processSingleInvoice = async (saleData, sessionId, index) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      // Validate product quantities first
      for (const p of saleData.products) {
        ...
        if (totalQty <= 0) {
          throw new Error(`Product "${p.productName}" has invalid quantity. Sales + Bonus must be > 0`);
        }
      }
      ...
      if (!BYPASS_STOCK_CHECK) {
        for (const p of saleData.products) {
          const salesQty = parseFloat(p.salesQty) || 0;
          if (salesQty > 0) {
            const stockCheck = await findProductStockInHand(p.productName, salesQty);
      ...
      // FIXED: Handle paymentStatus based on importStatus
      const importStatus = saleData.importStatus || saleData.paymentStatus || "pending";
      const paymentStatus = mapPaymentStatus(saleData.paymentStatus, importStatus);
      ...
      const newSale = new SaleSummary({
        ...
        importStatus: importStatus.toLowerCase(), // Store importStatus
        remark: saleData.remark || "",
      });

      await newSale.save({ session });

      // FIXED: Only add cash for Cash payment status, not for Credit
      if (paymentStatus === "Cash" && paidAmount > 0) {
        await addCashToMR({
    ...
    filteredSalesData.forEach((sale) => {
      ...
      sale.products.forEach((product) => {
        const row = worksheet.addRow({
          ...
          importStatus: sale.importStatus || "--",
          remark: sale.remark,
        });
...
router.post("/sales", async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      // For regular sales, importStatus is null
        ...
        [
          {
            ...
            totalAmount,
            paymentStatus: paymentStatus,
            importStatus: null, // Regular sales don't have importStatus
            remark: data.remark || "",
          },
        ],
        { session }
      ...
      // Only add cash for Cash payment status
      if (paymentStatus === "Cash" && paidAmount > 0) {
        await addCashToMR({
...
router.put("/sales/:id", async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      // Handle importStatus if provided
      const importStatus = saleData.importStatus || originalSale.importStatus;
      const paymentStatus = mapPaymentStatus(saleData.paymentStatus, importStatus);

      if (
        originalSale.paymentStatus !== paymentStatus ||
        originalSale.paidAmount !== paidAmount
      ) {
        if (originalSale.paymentStatus === "Cash" && originalSale.paidAmount > 0) {
          await removeCashFromMR({
      ...
      const updatedSale = await SaleSummary.findByIdAndUpdate(
        id,
        {
          ...
          dueAmount,
          paymentStatus: paymentStatus,
          importStatus: importStatus || originalSale.importStatus,

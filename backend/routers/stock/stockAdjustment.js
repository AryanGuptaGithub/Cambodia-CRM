...
router.post("/", protect, async (req, res) => {
  ...
  try {
    ...
    // ✅ FIX: Save unitCost and amount on the StockAdjustment document
    // so that DELETE logs can read them back for revert
    const adjustment = new StockAdjustment({
      ...
      unitCost: costPerBox, // ✅ ADDED
      amount: adjustmentAmount, // ✅ ADDED
    });
...
router.put("/:id", protect, allowAdminOnly, async (req, res) => {
  ...
  try {
    ...
    // ✅ FIX: Also update unitCost and amount on the document
    ...
    existingAdjustment.unitCost = costPerBox; // ✅ ADDED
    existingAdjustment.amount = adjustmentAmount; // ✅ ADDED
    ...
    await logActivity(req, {
      ...
      previousData: {
        ...
        unitCost: previousData.unitCost || 0,
    ...
    await emitEvent(req, { eventType: EVENT_TYPES.STOCK_ADJUSTED, status: 'FAILED', errorMessage: error.message });
...
router.delete("/:id", protect, allowAdminOnly, async (req, res) => {
  ...
  try {
    ...
    // ✅ FIX: Capture full adjustment data BEFORE deletion for revert support
    ...
    // ✅ FIX: Log full previousData including _id, productId, unitCost, amount
    await logActivity(req, {
      action: "DELETE",
      ...
      previousData: {
        _id: previousData._id, // ✅ Required for restore
        productId: previousData.productId, // ✅ Required for productName resolution
        productName: product.productName,
        adjustmentType: previousData.adjustmentType,
        boxQuantity: previousData.boxQuantity,
        unitCost: previousData.unitCost || 0, // ✅ Required for costPerBox on re-apply
        amount: previousData.amount || 0, // ✅ Required for amount on re-apply
        remarks: previousData.remarks,
...
router.delete("/bulk", protect, allowAdminOnly, async (req, res) => {
  ...
  try {
    ...
    // ✅ FIX: Capture full data for each adjustment including _id, unitCost, amount
    const previousDataArray = adjustments.map((adj) => {
      const raw = adj.toObject();
      return {
        _id: raw._id,
        productId: raw.productId,
        productName: adj.productId?.productName || "Unknown",
    ...
    // ✅ FIX: Log full previousData array with all required fields
    await logActivity(req, {
      action: "DELETE",
  ...
  try {
    ...
    for (const adj of adjustments) {
      if (
        ...
        adj.adjustmentType === "batch" ||
        adj.adjustmentType === "return"
      ) {

...
/* ==========================================================================
   ...
   ========================================================================== */
const updateReportInHandAfterStockTransfer = async (productName, boxQuantity, transferType) => {
  try {
    ...
    if (transferType === "send" && existingProduct.quantity.boxes < boxQuantity) {
      throw new Error(
...
/* ==========================================================================
   ...
   ========================================================================== */
const restoreReportInHandAfterStockTransferDeletion = async (productName, boxQuantity, transferType) => {
  try {
...
router.get("/stock-transfers/last-number", async (req, res) => {
  try {
    const lastTransfer = await StockTransfer.findOne().sort({ invoiceNo: -1 }).select("invoiceNo");
    ...
    res.status(500).json({ success: false, message: "Error fetching last stock transfer number" });
  }
});
...
router.get("/stock-transfers", async (req, res) => {
  try {
    const { type, page = 1, limit = 10, search, status, paymentStatus } = req.query;
    const filter = {};
    ...
    const [transfers, totalCount] = await Promise.all([
      StockTransfer.find(filter).sort({ createdAt: -1 }).limit(limitNum).skip(skip).lean(),
...
router.get("/stock-transfers/:id", async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(400).json({ success: false, message: "Invalid stock transfer ID" });
  }
...
router.post("/stock-transfers", async (req, res) => {
  ...
  try {
    const { invoiceNo, date, items, remarks, status, transferType, shipping, totalExpenses, grandTotal, destination, source } = req.body;
    ...
    if (transferType === "send" && !destination) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: "Destination is required for send transfers" });
    }

    if (transferType === "receive" && !source) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: "Source is required for receive transfers" });
    }
    ...
    for (const item of items) {
      if (!item.productId || !item.productName) {
        await session.abortTransaction();
        return res.status(400).json({ success: false, message: "Each item must have productId and productName" });
      }
      ...
      if (transferType === "send") {
        const existingProduct = await ReportInHand.findOne({ productName: item.productName });
        if (!existingProduct) {
          await session.abortTransaction();
          return res.status(400).json({ success: false, message: `Product "${item.productName}" not found in inventory` });
        }
    ...
    for (const item of items) {
      await updateReportInHandAfterStockTransfer(item.productName, parseFloat(item.boxQuantity), transferType);
    }
...
router.put("/stock-transfers/:id", async (req, res) => {
  ...
  try {
    ...
    for (const oldItem of transfer.items) {
      await restoreReportInHandAfterStockTransferDeletion(oldItem.productName, parseFloat(oldItem.boxQuantity), transfer.transferType);
    }

    // Apply new inventory update
    if (updateData.items) {
      for (const newItem of updateData.items) {
        await updateReportInHandAfterStockTransfer(newItem.productName, parseFloat(newItem.boxQuantity), updateData.transferType);
      }
    }

    const updatedTransfer = await StockTransfer.findByIdAndUpdate(id, updateData, { new: true, session });

    await session.commitTransaction();
    session.endSession();

    res.status(200).json({ success: true, message: "Stock transfer updated successfully", data: updatedTransfer });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    console.error("Error updating stock transfer:", error);
    res.status(500).json({ success: false, message: "Server error while updating stock transfer", error: error.message });
  }
});
...
router.delete("/stock-transfers/:id", async (req, res) => {
  ...
  try {
    ...
    for (const item of transfer.items) {
      await restoreReportInHandAfterStockTransferDeletion(item.productName, parseFloat(item.boxQuantity), transfer.transferType);
    }

    const deletedTransfer = await StockTransfer.findByIdAndDelete(id, { session });

    await session.commitTransaction();
    session.endSession();

    res.status(200).json({ success: true, message: "Stock transfer deleted successfully", data: deletedTransfer });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    console.error("Error deleting stock transfer:", error);
    res.status(500).json({ success: false, message: "Server error while deleting stock transfer", error: error.message });
  }
});
...
router.delete("/stock-transfers/bulk/delete", async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: "No stock transfer IDs provided" });
    }
    ...
    if (invalidIds.length > 0) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: "Invalid ID(s) provided", invalidIds });
    }
    ...
    const result = await StockTransfer.deleteMany({ _id: { $in: validIds } }, { session });

    if (result.deletedCount === 0) {
      await session.abortTransaction();
      return res.status(404).json({ success: false, message: "No stock transfers found to delete" });
    }
    ...
    res.status(500).json({ success: false, message: "Server error during bulk delete", error: error.message });
  }
});

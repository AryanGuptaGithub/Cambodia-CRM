  ...
  try {
    ...
    if (transferType === "send" && stockTransferToMr) {
      ...
      if (!mrStock) {
        ...
        // Existing MR stock - update or add each product
        for (const item of validNewItems) {
          const productName = item.productName.trim();
          const addQty = Number(item.boxQuantity) || 0;
          const lc = Number(item.lc) || 0;

          // Find if product already exists in the current stock (using the latest array)
          ...
          if (existingProductIndex >= 0) {
            ...
            await stockInMrHands.updateOne(
              {
                mrId: mrObjectId,
                "productsInHand.productName": mrStock.productsInHand[existingProductIndex].productName,
              },
              {
        ...
        // Refresh mrStock after updates? Not needed for this loop as we use updateOne with filters.
      }
    }
...
//       // --- FIX: Clear the MR stock document's productsInHand instead of deleting it ---
//       await stockInMrHands.updateOne(
//         { mrId: new mongoose.Types.ObjectId(targetMrId) },
//         { $set: { productsInHand: [] } },
//         { session }
//       );

//       // Now upsert will add all new items (the document exists, so it pushes each product)
...
//       // --- FIX: Delete the MR stock document to ensure a clean start ---
//       await stockInMrHands.deleteOne({ mrId: new mongoose.Types.ObjectId(targetMrId) }, { session });

//       // Now create fresh MR stock with the new items
//       await upsertProductsIntoMRHand(targetMrId, targetMrName, validNewItems, session);
//     }

...
const updateStockInMRHand = async (mrName, productId, productName, boxQuantity, operation) => {
  ...
  if (!result) {
    ...
    const updatedDoc = await StockInMRHand.findOneAndUpdate(
      { mrName, "products.productId": productId, "products.boxQuantity": { $lte: 0 } },
      {
  ...
  try {
    ...
    if (data.transferType === "send") {
      ...
      for (const item of data.items) {
        const lcValue = await deductFromReportInHand(item.productName, item.boxQuantity);
        
        // Update MR hand stock (add product to array)
        await updateStockInMRHand(mrName, item.productId, item.productName, item.boxQuantity, 'add');
      }
    }
    ...
    if (data.transferType === "receive") {
      ...
      for (const item of data.items) {
        ...
        await updateStockInMRHand(mrName, item.productId, item.productName, item.boxQuantity, 'subtract');
        ...
        const productStock = await ReportInHand.findOne({ productName: item.productName });
...
router.get("/stock-transfers-to-mr", async (req, res) => {
  try {
    ...
    const transfersWithCosts = transfers.map((transfer) => {
     ...
     console.log("123320", transfersWithCosts);
...
router.get("/stock-in-mr-hand", async (req, res) => {
  try {
    ...
    const flattenedStock = stock.map(mrStock => {

...
router.put('/purchase-return/:id', async (req, res) => {
  try {
    ...
    if (existingReturn) {
      return res.status(400).json({
        success: false,
        message: 'Another purchase return for this invoice and product already exists'
      });
    }
...
router.delete('/purchase-return/:id', async (req, res) => {
  try {
    const deletedPurchaseReturn = await PurchaseReturn.findByIdAndDelete(req.params.id);

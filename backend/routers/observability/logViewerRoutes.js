...
// Returns live dashboard card values right now.
// The frontend calls this when opening an event drawer.
// ─────────────────────────────────────────────
router.get('/dashboard-snapshot', async (req, res) => {
  try {
    console.log('[observability] GET /dashboard-snapshot requested');
    const snapshot = await captureDashboardSnapshot();
    if (!snapshot) {
      return res.status(503).json({ success: false, error: 'Snapshot unavailable' });
    }
    res.json({ success: true, data: snapshot });
  } catch (err) {
    console.error('[observability] GET /dashboard-snapshot error:', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

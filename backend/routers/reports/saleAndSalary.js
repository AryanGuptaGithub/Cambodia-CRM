...
const normalizeMrName = (name) => {
  ...
  let normalized = name
    ...
    .trim()
    .toLowerCase();
  if (normalized.includes("makara")) normalized = "makara";
  if (normalized.includes("phanda")) normalized = "phanda";
  return normalized;
};
...
// Previously this returned 1000 when totalExpense === 0, which showed as
// +1000.00% in the UI. Now it caps the result at 100%.
const calculatePerformance = (profit, totalExpense) => {
  if (totalExpense === 0) return profit > 0 ? 100 : 0;
  const raw = (profit / totalExpense) * 100;
  return Math.min(raw, 100); // never exceed 100%
};
...
const fetchSalesSalaryData = async (params) => {
  try {
    ...
    if (startDate && endDate) {
      ...
      switch (dateFilter) {
        case "today": {
          ...
          salesDateConditions.recordingDate = { $gte: today, $lt: tomorrow };
          break;
        }
        case "currentMonth": {
          const firstDay = new Date(year, month, 1);
          const lastDay = new Date(year, month + 1, 0);
          salesDateConditions.recordingDate = { $gte: firstDay, $lte: lastDay };
          break;
        }
    ...
    // Group by normalised name to avoid duplicate MR rows
    const groupedSales = {};
    salesAggregate.forEach((record) => {
      ...
      if (!groupedSales[normalizedName]) {
        ...
        groupedSales[normalizedName].saleCount +=
          parseInt(record.saleCount) || 0;
        ...
        if (
          record.lastSaleDate &&
          (!groupedSales[normalizedName].lastSaleDate ||
        ...
        if (
          record.mrName &&
          !groupedSales[normalizedName].originalNames.includes(record.mrName)
        ) {
          groupedSales[normalizedName].originalNames.push(record.mrName);
        }
        if (
          parseFloat(record.sale) >
          parseFloat(groupedSales[normalizedName].records[0].sale)
        ) {
    ...
    allStaffMembers.forEach((staff) => {
      ...
      if (originalName) {
        ...
        staffMap.normalizedNameToId[normalizeMrName(originalName)] = staffId;
        staffMap.nameToId[originalName.toLowerCase().trim()] = staffId;
      }
    });
    ...
    if (allStaffIds.length > 0) {
        ...
        switch (dateFilter) {
          ...
          case "janToPreviousMonth":
            if (now.getMonth() === 0) {
              ...
              for (let i = 1; i <= 12; i++) {
                payrollPeriods.push(
                  `${prevYear}-${i.toString().padStart(2, "0")}`,
                );
              }
            } else {
      ...
      // tourExpense  → travelAllowance  (the actual field name in Payroll)
      // incentive    → incentive        (kept separate, NOT summed into allowance)
      // allowance    → totalAllowance   (does NOT include incentive)
      // otherExpense → REMOVED entirely
      payrollAggregate = await Payroll.aggregate([
        { $match: payrollMatchConditions },
        {
          $group: {
            ...
            // travelAllowance is the Payroll field that represents Tour Expense
            tourExpense: { $sum: { $ifNull: ["$travelAllowance", 0] } },
            payrollCount: { $sum: 1 },
          },
        },
      ]);
    ...
    // Summary: totalExpense = salary + incentive + allowance + tourExpense
    // (otherExpense removed)
    const totalPayrollSummary = payrollAggregate.reduce(
      (acc, payroll) => ({
        totalSalary: acc.totalSalary + (parseFloat(payroll.salary) || 0),
        totalIncentive:
          acc.totalIncentive + (parseFloat(payroll.incentive) || 0),
        totalAllowance:
          acc.totalAllowance + (parseFloat(payroll.allowance) || 0),
        totalTourExpense:
          acc.totalTourExpense + (parseFloat(payroll.tourExpense) || 0),
      }),
      {
        totalSalary: 0,
        totalIncentive: 0,
    ...
    let combinedData = salesAggregateGrouped.map((record) => {
      ...
      if (
        normalizedSalesName &&
        staffMap.normalizedNameToId[normalizedSalesName]
      ) {
        matchedStaffId = staffMap.normalizedNameToId[normalizedSalesName];
      }
      if (!matchedStaffId && normalizedSalesName) {
        for (const [staffNormalizedName, staffId] of Object.entries(
          staffMap.normalizedNameToId,
        )) {
          if (
            staffNormalizedName.includes(normalizedSalesName) ||
      ...
      if (!matchedStaffId && record.originalNames?.length > 0) {
      ...
      const payroll = matchedStaffId
        ? payrollByStaffId[matchedStaffId] || {}
        : {};

      const salary = parseFloat(payroll.salary) || 0;
      const incentive = parseFloat(payroll.incentive) || 0;
      const allowance = parseFloat(payroll.allowance) || 0; // does NOT include incentive
      const tourExpense = parseFloat(payroll.tourExpense) || 0; // from travelAllowance field
      const sale = parseFloat(record.sale) || 0;
      const profit = parseFloat(record.profit) || 0;

      // totalExpense = salary + incentive + allowance + tourExpense (no otherExpense)
    ...
    // Add staff who have payroll entries but no sales records
    Object.keys(staffMap.normalizedNameToId).forEach((normalizedName) => {
      ...
      if (
        payroll &&
        !combinedData.find(
          (item) => normalizeNameInJS(item.mrName) === normalizedName,
        )
      ) {
        const salary = parseFloat(payroll.salary) || 0;
        const incentive = parseFloat(payroll.incentive) || 0;
        ...
        combinedData.push({
          srDate: new Date(),
          mrName: staffEntry?.medicalRepName || normalizedName,
          mrId: staffId,
          sale: 0,
          profit: 0,
          salary,
    ...
    if (isExport) {
      ...
      const paginatedData = combinedData.slice(
        skip,
        Math.min(skip + limitNum, totalRecords),
      );
      return {
        success: true,
        data: { summary, records: paginatedData },
        pagination: {
          currentPage: pageNum,
          totalPages,
          totalRecords,
...
router.get("/export", async (req, res) => {
  try {
    ...
    records.forEach((record, index) => {
      ...
      salarySaleCell.numFmt = '0.00"%"';
      salarySaleCell.font = {
        color: {
          argb:
            (parseFloat(record.salarySaleRatio) || 0) >= 0
              ? "FF16A34A"
              : "FFDC2626",
        },
      };
      ...
      perfCell.font = {
        color: {
          argb:
            (parseFloat(record.performance) || 0) >= 0
              ? "FF16A34A"
              : "FFDC2626",
        },
      };
    ...
    const summaryRow = worksheet.addRow({
      ...
      salarySaleRatio:
        summary.totalExpense > 0
          ? ((summary.totalProfit - summary.totalExpense) /
              summary.totalExpense) *
            100
          : 0,
      performance:
        summary.totalExpense > 0
          ? Math.min((summary.totalProfit / summary.totalExpense) * 100, 100)
          : 0,
    });
    ...
    ["sale", "profit", "salary", "totalExpense"].forEach((col) => {
      summaryRow.getCell(col).numFmt = "$#,##0.00";
    });
    ["salarySaleRatio", "performance"].forEach((col) => {
      summaryRow.getCell(col).numFmt = '0.00"%"';
    });
    ...
    const filename = `sales-salary-ratio-report-${new Date().toISOString().split("T")[0]}.xlsx`;
    res.setHeader(
      "Content-Type",

...
// Rules:
//   < 4 h  → absent / short (not recorded as attendance)
//   4–<7 h → half day  (expected = 4 h; extras = worked − 4 h)
//   ≥ 7 h  → full day  (expected = 8 h; extras = worked − 8 h, clamped ≥ 0)
...
// ─── DELETE /attendance/date/:userId/:date ────────────────────────────────────
    ...
    if (leaveType && !validLeaveTypes.includes(leaveType)) {
      return res
        ...
        .json({
          success: false,
          message:
            "Invalid leave type. Must be 'paid', 'unpaid', or 'swapleave'",
        });
    }
...
// Half-day logic: 4 h ≤ worked < 7 h → half day (expected 4 h, extras from 4 h base)
//                 worked ≥ 7 h        → full day (expected 8 h, extras from 8 h base)
router.post("/attendance/record", async (req, res) => {
  try {
    ...
    // Determine attendance type using shared helper
    const { type: attendanceType, expectedMinutes } =
      getAttendanceType(totalMinutesWorked);

    // Extra hours: anything above the expected baseline (4 h for half, 8 h for full)
...
// ─── POST /attendance/convert-to-leave ───────────────────────────────────────

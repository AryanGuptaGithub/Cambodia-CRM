...
function Purchase() {
  ...
  // ── Download All Excel (inline, no separate component needed) ──────────
const handleDownloadAllExcel = async () => {
  setIsDownloadingAll(true);
  try {
    const token = localStorage.getItem("token");
    // Use POST, no request body → all records
    const response = await axios.post(
      `${backendUrl}/api/purchase/download-excel`,
      {},   // empty body – backend will not filter by date
      {
    ...
    // Handle successful blob response (Excel file)
    ...
    // If the backend returns JSON (e.g., "no purchases found"), error.response.data will be a Blob.
    // We need to read it as text to display the message.
  ...
  const ImportModal = ({ isOpen, onClose, isSampleFile }) => {
    ...
    useEffect(() => {
      ...
      if (existingInvoices.length > 0) {
        ...
        parsedData.forEach((group, idx) => {
          if (existingKeys.has(getRowKey(group))) duplicateIndices.add(idx);
        });
      }
      setDuplicateRows(
      ...
      reader.onload = (evt) => {
        try {
          ...
          dataRows.forEach((row, idx) => {
            ...
            const invoiceDate = parseExcelDateValue(getValue(obj, ["Invoice Date"]));
            const deliveryNumber =
              getValue(obj, ["Delivery Number", "Delivery No.", "Delivery No"])
                ?.toString()
                .trim() || invoiceNumber;
            const receivedDate =
              parseExcelDateValue(getValue(obj, ["Received Date"])) || invoiceDate;
            const expiryDate = parseExcelDateValue(getValue(obj, ["Expiry Date"]));
            const remarks =
      ...
      <div className="container">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full">
          <div className="flex gap-3 items-center flex-wrap">
            ...
            {/* ── Download All Excel (only when flag is true) ── */}

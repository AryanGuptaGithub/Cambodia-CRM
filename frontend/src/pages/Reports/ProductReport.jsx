...
// Pagination component (responsive like DailyReports)
const Pagination = ({ total, page, onPage, isMobileView }) => {
...
const StatCard = ({
  ...
  return (
    <div
      className={`rounded-xl border ${isMobileView ? "p-3" : "p-4"} flex flex-col gap-1 ${colors[color]}`}
    >
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-wide opacity-70">
          {label}
        </p>
        {Icon && <Icon size={isMobileView ? 14 : 16} className="opacity-50" />}
      </div>
      <p className={`${isMobileView ? "text-xl" : "text-2xl"} font-bold`}>
        {value}
      </p>
      {sub && <p className="text-xs opacity-60 hidden sm:block">{sub}</p>}
    </div>
  );
};

// CustomRangeModal component (responsive)
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-auto overflow-hidden">
        ...
        <div className="px-4 sm:px-6 py-2 sm:py-3 bg-indigo-50 flex items-center justify-between text-sm">
          ...
          <div className="text-gray-400 text-base sm:text-lg">→</div>
          <div className="text-center">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="text-xs sm:text-sm font-semibold text-gray-800 border-0 bg-transparent focus:outline-none cursor-pointer"
              >
                {MONTHS.map((m, i) => (
            ...
            {Array.from({ length: days }, (_, i) => i + 1).map((day) => {
              ...
              return (
                <button
                  ...
                  className={`h-8 sm:h-9 w-full text-xs sm:text-sm font-medium transition-all rounded-lg
        ...
        <div className="px-3 sm:px-6 pb-3 flex flex-wrap gap-2">
          {presets.map((p) => (
            <button
              key={p.label}
              onClick={p.fn}
              className="text-xs px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-700 transition-colors"
            >
              {p.label}
            </button>
          ))}
        </div>
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100 flex items-center justify-end gap-3">
          <button
            onClick={onClose}
            className="px-3 sm:px-4 py-1.5 sm:py-2 text-sm text-gray-600 font-medium rounded-lg hover:bg-gray-100"
          >
            Cancel
          </button>
          <button
            ...
            className="px-4 sm:px-5 py-1.5 sm:py-2 text-sm font-semibold bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white rounded-lg transition-colors"
          >
            Apply Range
          </button>
        </div>
...
// DateFilterBar component (responsive)
const DateFilterBar = ({
  filterMode,
  year,
  month,
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4 flex flex-wrap gap-2 sm:gap-3 items-center shadow-sm">
      {recentMonths.map((rm) => (
        <button
          key={`${rm.month}-${rm.year}`}
          onClick={() => onMonthClick(rm.month, rm.year)}
          className={`px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm rounded-lg font-medium transition-all border ${
      ...
      <select
        ...
        className={`border rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-300 cursor-pointer ${
          filterMode === "year"
      ...
      <button
        ...
        className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm rounded-lg font-medium border transition-all ${
          filterMode === "custom"
            ? "bg-indigo-600 text-white border-indigo-600 shadow"
            : "bg-white text-gray-600 border-gray-200 hover:border-indigo-300 hover:text-indigo-600"
        }`}
      ...
      <button
        onClick={onApply}
        disabled={loading}
        className="flex items-center gap-1 sm:gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium ml-auto"
      >
        {loading ? (
...
// TableSkeleton (responsive)
const TableSkeleton = ({ isMobileView }) => (
  <div className="space-y-4">
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
      {[1, 2, 3, 4, 5].map((i) => (
        <div
          key={i}
          className="h-20 sm:h-24 rounded-xl bg-gray-100 animate-pulse"
        />
      ))}
    </div>
...
// AllTab component (responsive)
const AllTab = forwardRef(
  ({ data, loading, search, onExport, isMobileView }, ref) => {
    ...
    if (loading) return <TableSkeleton isMobileView={isMobileView} />;
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-[800px] w-full text-sm bg-white text-center">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {[
                    ...
                    <th
                      key={h}
                      className="px-3 sm:px-4 py-2 sm:py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {paginated.length === 0 ? (
                  ...
                  paginated.map((row, i) => (
                    ...
                    >
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-400">
                        {(page - 1) * PAGE_SIZE + i + 1}
                      </td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 font-medium text-gray-800 text-left">
                      ...
                      <td className="px-3 sm:px-4 py-2 sm:py-3">
                        {fmtInt(row.totalSalesQty)}
                      </td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-amber-600 font-medium">
                      ...
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-green-700 font-semibold">
                        ${fmt(row.periodSales)}
                      </td>
                      <td
                        className={`px-3 sm:px-4 py-2 sm:py-3 font-semibold ${row.profitAmount >= 0 ? "text-green-700" : "text-red-600"}`}
                      >
              ...
              {filtered.length > 0 && page === totalPages && (
                <tfoot className="bg-gray-50 border-t-2 border-gray-300">
                  <tr>
                    <td
                      colSpan={3}
                      className="px-3 sm:px-4 py-2 sm:py-3 font-bold text-gray-700"
                    >
                      Grand Total
                    </td>
                    <td className="px-3 sm:px-4 py-2 sm:py-3 text-center font-bold">
                      {fmtInt(totals.salesQty)}
                    </td>
                    <td className="px-3 sm:px-4 py-2 sm:py-3 text-center font-bold text-amber-600">
                    ...
                    <td className="px-3 sm:px-4 py-2 sm:py-3" />
                    <td className="px-3 sm:px-4 py-2 sm:py-3 font-bold text-green-700">
                      ${fmt(totals.netAmount)}
                    </td>
                    <td
                      className={`px-3 sm:px-4 py-2 sm:py-3 font-bold ${totals.profitLoss >= 0 ? "text-green-700" : "text-red-600"}`}
                    >
                      ${fmt(totals.profitLoss)}
                    </td>
                    <td colSpan={2} className="px-3 sm:px-4 py-2 sm:py-3" />
                  </tr>
                </tfoot>
              )}
            </table>
          ...
          {filtered.length > PAGE_SIZE && (
            <div className="border-t border-gray-100 bg-white px-2 sm:px-4">
              <Pagination
...
// MRWiseTab component (responsive)
const MRWiseTab = forwardRef(
  ({ data, loading, search, onExport, isMobileView }, ref) => {
    ...
    const totals = useMemo(
      () => ({
        ...
        profitLoss: filtered.reduce(
          (s, mr) => s + (mr.totalProfitLoss || 0),
          0,
        ),
      }),
      [filtered],
    ...
    return (
      <div className="space-y-4">
            ...
            paginated.map((mr) => (
              ...
              >
                <button
                  ...
                  className="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 sm:px-5 py-3 sm:py-4 bg-indigo-50 hover:bg-indigo-100 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xs font-bold">
                ...
                {expanded[mr.mrName] && (
                  <div className="overflow-x-auto">
                    <table className="min-w-[700px] w-full text-sm bg-white text-center">
                      <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                          {[
                            "#",
                            ...
                            <th
                              key={h}
                              className="px-3 sm:px-4 py-2 sm:py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap"
                            >
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {(mr.products || [])
                          ...
                          .map((p, i) => (
                            ...
                            >
                              <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-gray-400">
                                {i + 1}
                              </td>
                              <td className="px-3 sm:px-4 py-2 sm:py-2.5 font-medium text-gray-800 text-left">
                                {capitalizeFirstLetter(p.productName)}
                              </td>
                              <td className="px-3 sm:px-4 py-2 sm:py-2.5">
                                {fmtInt(p.salesQty)}
                              </td>
                              <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-amber-600">
                                {fmtInt(p.bonusQty)}
                              </td>
                              <td className="px-3 sm:px-4 py-2 sm:py-2.5 font-semibold">
                                {fmtInt(p.totalQty)}
                              </td>
                              <td className="px-3 sm:px-4 py-2 sm:py-2.5">
                                ${fmt(p.avgPrice)}
                              </td>
                              <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-green-700 font-semibold">
                                ${fmt(p.netSellingAmount)}
                              </td>
                              <td
                                className={`px-3 sm:px-4 py-2 sm:py-2.5 font-semibold ${p.profitLoss >= 0 ? "text-green-700" : "text-red-600"}`}
                              >
                                ${fmt(p.profitLoss)}
                              </td>
                            </tr>
                      ...
                      <tfoot className="bg-gray-50 border-t border-gray-300">
                        <tr>
                          <td
                            colSpan={2}
                            className="px-3 sm:px-4 py-2 sm:py-2.5 font-bold text-gray-700"
                          >
                            MR Total
                          </td>
                          <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-center font-bold">
                            {fmtInt(mr.totalSalesQty)}
                          </td>
                          <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-center font-bold text-amber-600">
                            {fmtInt(mr.totalBonusQty)}
                          </td>
                          <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-center font-bold">
                            {fmtInt(mr.totalQty)}
                          </td>
                          <td className="px-3 sm:px-4 py-2 sm:py-2.5 font-bold">
                            ${fmt(mr.avgPrice)}
                          </td>
                          <td className="px-3 sm:px-4 py-2 sm:py-2.5 font-bold text-green-700">
                            ${fmt(mr.totalNetAmount)}
                          </td>
                          <td
                            className={`px-3 sm:px-4 py-2 sm:py-2.5 font-bold ${mr.totalProfitLoss >= 0 ? "text-green-700" : "text-red-600"}`}
                          >
        ...
        {filtered.length > PAGE_SIZE && (
          <div className="bg-white border border-gray-200 rounded-xl px-2 sm:px-4 shadow-sm">
            <Pagination
              total={filtered.length}
              page={page}
              onPage={setPage}
...
// SampleWiseTab (responsive)
const SampleWiseTab = forwardRef(
  ({ data, loading, search, onExport, isMobileView }, ref) => {
    const [expanded, setExpanded] = useState({});
    const [page, setPage] = useState(1);
    ...
    return (
      <div className="space-y-4">
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 sm:px-5 py-3 sm:py-4 bg-purple-50 hover:bg-purple-100 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-purple-600 text-white flex items-center justify-center flex-shrink-0">
                  ...
                  {expanded[day.dateKey] && (
                    <div className="overflow-x-auto">
                      <table className="min-w-[900px] w-full text-sm bg-white text-center">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            <th className="px-3 sm:px-4 py-2 sm:py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-12">
                              SR
                            </th>
                            <th className="px-3 sm:px-4 py-2 sm:py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                              Customer
                            </th>
                            <th className="px-3 sm:px-4 py-2 sm:py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                              Product
                              ...
                              return (
                                ...
                                >
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-center">
                                    <span className="inline-flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-purple-100 text-purple-700 text-xs font-bold">
                                      {entry.srNo || i + 1}
                                    </span>
                                  </td>
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5">
                                  ...
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-gray-700 text-xs sm:text-sm">
                                    {capitalizeFirstLetter(entry.productName)}
                                  </td>
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5">
                                    <span className="px-1.5 sm:px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold">
                                  ...
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5">
                                    {hasSale ? (
                                      <button
                                        onClick={() =>
                                          handleOrderQtyClick(entry)
                                        }
                                        className="px-1.5 sm:px-2.5 py-0.5 rounded-full bg-green-100 text-green-700 text-xs font-semibold hover:bg-green-200 focus:outline-none cursor-pointer transition-colors"
                                      ...
                                      <span className="px-1.5 sm:px-2.5 py-0.5 rounded-full bg-rose-100 text-rose-600 text-xs font-semibold">
                                        <MinusCircle
                                          size={10}
                                          className="inline mr-1"
                                        />
                                  ...
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-green-700 font-semibold text-xs sm:text-sm">
                                    {hasSale
                                      ? `$${fmt(entry.saleAmount)}`
                                      : "—"}
                                  </td>
                                  <td
                                    className={`px-3 sm:px-4 py-2 sm:py-2.5 font-semibold text-xs sm:text-sm ${
                                      entry.profit >= 0
                                        ? "text-green-700"
                                        : "text-red-600"
                                    }`}
                                  ...
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-gray-600 text-xs sm:text-sm">
                                    {entry.mrName}
                                  </td>
                                  <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-gray-400 text-xs max-w-[160px] truncate">
                                    {entry.remark || "—"}
                                  </td>
                                </tr>
                              );
                        ...
                        <tfoot className="bg-gray-50 border-t border-gray-300">
                          <tr>
                            <td
                              colSpan={4}
                              className="px-3 sm:px-4 py-2 sm:py-2.5 font-bold text-gray-700 text-xs sm:text-sm text-left"
                            >
                              Day Total
                            </td>
                            <td className="px-3 sm:px-4 py-2 sm:py-2.5 text-center font-bold text-green-700">
                              {fmtInt(dayTotalOrder)}
                            </td>
                            <td
                              colSpan={4}
        ...
        {/* Sales Details Modal - Responsive */}
        {salesModal.open &&
          ReactDOM.createPortal(
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden mx-auto">
                <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-800">
                    Sale Details for{" "}
                    {capitalizeFirstLetter(salesModal.productName)}
                  </h3>
                  <button
                ...
                <div className="px-4 sm:px-6 py-2 sm:py-3 bg-gray-50">
                  <p className="text-xs sm:text-sm text-gray-600">
                    ...
                    <span className="font-medium">
                      {capitalizeFirstLetter(salesModal.customerName)}
                    </span>
                  </p>
                </div>
                <div className="overflow-y-auto p-4 sm:p-6 max-h-96">
                  {salesModal.sales.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-[500px] w-full text-xs sm:text-sm text-left">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            <th className="px-2 sm:px-4 py-2 text-xs font-semibold text-gray-500 uppercase">
                              #
                            </th>
                            <th className="px-2 sm:px-4 py-2 text-xs font-semibold text-gray-500 uppercase">
                              Invoice #
                        ...
                        <tbody>
                          {salesModal.sales.map((sale, idx) => (
                            <tr key={idx} className="border-b border-gray-100">
                              ...
                              <td className="px-2 sm:px-4 py-2 font-mono text-xs">
                                {sale.invoiceNumber || "—"}
                              </td>
                              ...
                              <td className="px-2 sm:px-4 py-2">
                                {fmtInt(sale.salesQty)}
                              </td>
                              <td className="px-2 sm:px-4 py-2">
                                {fmtInt(sale.bonusQty)}
                              </td>
                              <td className="px-2 sm:px-4 py-2 font-semibold">
                                {fmtInt(sale.totalQty)}
                              </td>
                              <td className="px-2 sm:px-4 py-2 text-green-700">
                                ${fmt(sale.amount)}
                              </td>
                              <td
                                className={`px-2 sm:px-4 py-2 ${sale.profit >= 0 ? "text-green-700" : "text-red-600"}`}
                              >
                                ${fmt(sale.profit)}
                              </td>
                            </tr>
                ...
                <div className="flex justify-end px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100">
                  <button
                    onClick={() => setSalesModal({ open: false, sales: [] })}
                    className="px-3 sm:px-4 py-1.5 sm:py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg text-xs sm:text-sm font-medium"
                  >
                    Close
                  </button>
                </div>
...
const ProductSalesReport = () => {
  ...
  return (
    <div
      className={`${isMobileView ? "p-3 pb-20" : "p-3 sm:p-6"} space-y-4 sm:space-y-6 min-h-screen bg-gray-50 relative`}
    >
      {/* ── Sidebar (mobile only) ── */}
      {isMobileView && (
        <Sidebar
      ...
      {isMobileView && (
        <div className="flex justify-between items-center mb-3 bg-gray-200 border-gray-200 p-2 rounded-2xl">
          ...
          <div className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
            Total Records: {allData.length + mrData.length + sampleData.length}
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="flex gap-1 bg-white border border-gray-200 rounded-xl p-1.5 shadow-sm overflow-x-auto">
          {TABS.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${
                  activeTab === tab.key
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 w-full sm:w-64"
            />
          </div>
          {/* Export button - hidden on mobile, visible on desktop */}
          <button
            onClick={handleExportCurrentTab}
            className="hidden sm:flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium"
          >

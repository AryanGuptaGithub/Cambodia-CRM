...
const parseExcelDateValue = (dateVal) => {
  if (!dateVal) return "";
...
const ImportModal = ({ isOpen, onClose, isSampleFile }) => {
  ...
  useEffect(() => {
    if (isOpen) fetchExistingProducts();
  }, [isOpen]);
  ...
  const fetchExistingProducts = async () => {
    ...
    try {
      ...
      if (Array.isArray(res.data)) setExistingProducts(res.data);
    } catch (error) {
      console.error("Failed to fetch existing products", error);
      showToast(
        "error",
        "Could not load existing products for duplicate check",
      );
    } finally {
    ...
    reader.onload = (evt) => {
      try {
        ...
        for (let i = 0; i < Math.min(rows.length, 15); i++) {
          ...
          if (
            requiredHeaders.filter((h) => cleanedRow.includes(h)).length >=
            requiredHeaders.length * 0.8
          ) {
            headerRowIndex = i;
            break;
          }
        ...
        rows[headerRowIndex].forEach((headerText, colIndex) => {
          ...
          if (allHeaders.includes(cleaned)) headersMap[colIndex] = cleaned;
        });
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (!productName && !type && !packing && !supplierName) {
            rowErrors.push(
              `Row ${headerRowIndex + idx + 2}: Missing product data — skipped`,
            );
            return;
          }
          let licenseValidityDate = "";
          const dateVal = obj["drug registration license validity date"];
          if (dateVal) licenseValidityDate = parseExcelDateValue(dateVal);
          validRows.push({
            productName,
            type,
            packing,
...
const Product = () => {
  ...
  const toggleSelectAll = useCallback(
    (checked) => {
      setSelected(checked ? products.map((p) => ({ id: p._id })) : []);
    },
    [products],
  );
  ...
  const handleTypeChange = useCallback(
    (selectedValue) => setForm((prev) => ({ ...prev, type: selectedValue })),
    [],
  );
  ...
  const handlePackingChange = useCallback(
    (selectedValue) => setForm((prev) => ({ ...prev, packing: selectedValue })),
    [],
  );
    ...
    try {
      ...
      allProducts.forEach((p) => {
        rows.push([
          ...
          p.supplierName || "",
          p.drugLicense || "",
          p.licenseValidityDate ? p.licenseValidityDate + " 00:00:00" : "",
          p.remarks || "",
        ]);
      });
      ...
      {isMobileView && (
        <div className="flex justify-between items-center mb-4 rounded-2xl p-2 bg-gray-200">
          ...
          <div className="bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full text-sm font-medium shadow-sm">
            Total Product: {paginationInfo.totalItems}
          </div>
        </div>
      )}
      ...
      {!isMobileView && (
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            {isSampleDownloadFile && <SampleExcelDownloadProduct />}
            {selected.length > 0 && (
              <button
                onClick={handleDeleteSelected}
                className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-sm"
              >
                <Trash2 size={16} />
        ...
        <table className="min-w-[800px] md:min-w-full w-full border-collapse bg-white rounded-2xl text-center">
          <thead className="bg-gray-100 text-gray-700 border-b">
            <tr>
              ...
              {isMobileView && (
                <th className="p-2 text-xs font-medium">Product Name</th>
              )}
                      ...
                      <DatePicker
                        selected={
                          form.licenseValidityDate
                            ? (() => {
                                const [y, m, d] = form.licenseValidityDate
                                  .split("-")
                                  .map(Number);
                                return new Date(
                        ...
                        onChange={(date) => {
                          if (date) {
                            ...
                            setForm({
                              ...form,
                              licenseValidityDate: `${year}-${month}-${day}`,
                            });
                          } else {

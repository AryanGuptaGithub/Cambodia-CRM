...
const StockAdjustment = () => {
  ...
  // Selection handlers - FIXED: Use all filtered adjustments for select all
  const handleSelect = (id) => {
    setSelectedIds((prev) =>
      prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id]
    );
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      // Select all IDs from the entire filtered dataset, not just current page
      const allFilteredIds = filteredAdjustments.map((adj) => adj._id);
  ...
  const isAllOnPageSelected = paginatedAdjustments.length > 0 && 
    paginatedAdjustments.every(adj => selectedIds.includes(adj._id));

  // Check if some items on current page are selected
  const isSomeOnPageSelected = paginatedAdjustments.some(adj => selectedIds.includes(adj._id));

  // CRUD operations - FIXED: Bulk delete with proper IDs
  const handleBulkDelete = async () => {
  ...
  const handleEdit = (adjustment) => {
    ...
    setFormData({
      ...
      quantityPerCarton: adjustment.quantityPerCarton || 0, // Keep existing open pieces value
      adjustmentType: adjustment.adjustmentType,
      notes: adjustment.notes,
    });
    setModalVisible(true);
  ...
  // FIXED: Don't reset open pieces when product changes
  const handleFormChange = (field, value) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    ...
    // When product is selected, ONLY update pieces per box display, don't change open pieces
    // This preserves the user's input for open pieces
  };
  ...
  const handleModalCancel = () => {
    setFormData({
      product: "",
      boxQuantity: 0,
      quantityPerCarton: 0, // Reset to 0 when modal closes
      adjustmentType: "add",
      notes: "",
    });
    setModalVisible(false);
  ...
  return (
    <div className="max-w-8xl p-6 bg-white rounded-xl shadow">
      ...
      {/* Table */}
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm border border-gray-200 rounded-lg overflow-hidden text-center">
            ...
            <tr>
              <th className="p-3">
                <input
                  ...
                  ref={(input) => {
                    if (input) {
                      input.indeterminate = isSomeOnPageSelected && !isAllOnPageSelected;
                    }
                  }}
                  onChange={handleSelectAll}
                  className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
              ...
              <th className="p-3">Box Quantity</th>
              <th className="p-3">Open Pieces</th>
            ...
            {paginatedAdjustments.length === 0 ? (
              ...
              paginatedAdjustments.map((adj) => (
                ...
                >
                  ...
                  <td className="p-3">{adj.boxQuantity}</td>
                  <td className="p-3 text-gray-600">
                    {adj.quantityPerCarton}
                  </td>
                  <td
                    className={`p-3 font-medium ${getQuantityColor(
                      adj.totalQuantity
                    )}`}
                  >
                    {getQuantityDisplay(adj.totalQuantity)}
        ...
        {selectedIds.length > 0 && (
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
            <p className="text-sm text-blue-700">
              {selectedIds.length} adjustment(s) selected
            </p>
          </div>
        )}
              ...
              <form onSubmit={handleModalSubmit} className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Open Pieces - FIXED: Now preserves user input */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Open Pieces <span className="text-red-500">*</span>
                      </label>

...
const bypassStockCheck = import.meta.env.VITE_BYPASS_STOCK_CHECK === "true";
    ...
    try {
      ...
      if (sessionId) {
        try {
          ...
          console.warn("Could not fetch failed invoices:", fetchError.message);
        }
      }
...
const Sales = () => {
  ...
  const [hasPurchaseEntry, setHasPurchaseEntry] = useState(false);
  ...
  useEffect(() => {
    const checkPurchaseEntry = async () => {
      try {
        const res = await axios.get(`${backendUrl}/api/reports/inhand/count`);
        const totalItems = res.data?.totalItems || 0;
        setHasPurchaseEntry(totalItems > 0);
      } catch (err) {
        console.warn("Could not check purchase entry:", err);
        setHasPurchaseEntry(false);
      }
    };
    checkPurchaseEntry();
  }, []);
  ...
  const actionsDisabled =
    showMRCustomerWarning || (!bypassStockCheck && !hasPurchaseEntry);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.delete(`${backendUrl}/api/sales/delete-batch`, {
  ...
  const filteredSales = useMemo(() => {
    ...
    return sales.filter((sale) => {
      ...
      return ["invoiceNumber", "customerName", "mrName"].some((field) =>
        (sale[field] ?? "").toString().toLowerCase().includes(lowerSearch)
      );
    });
  ...
  const toggleSelect = (sale) => {
    setSelected((prev) => {
      const exists = prev.some((s) => s.id === sale._id);
      return exists
        ? prev.filter((s) => s.id !== sale._id)
        : [...prev, { id: sale._id }];
    });
  };
  ...
  const deleteSale = async (sale) => {
    if (!sale._id) return;
    const confirm = await confirmDialog({
      text: `Delete ${sale.invoiceNumber}?`,
      icon: "warning",
    });
    if (confirm.isConfirmed) {
      try {
        await axios.delete(`${backendUrl}/api/sales/${sale._id}`);
  ...
  const handleImportSuccess = () => {
    setTimeout(() => fetchSaleSummaries(), 1000);
  };
  ...
  return (
    <div className="p-6">
      ...
      {/* Edit & View Modals – your full code here */}
        ...
        <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
          <div className="flex gap-3 items-center">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl shadow-md transition-all ${
                actionsDisabled
          ...
          {sales.length > 0 && (
            <SaleExcelDownload type="sales" buttonText="Download Sales Excel" />
          )}
        </div>

        {!bypassStockCheck && !hasPurchaseEntry && !showMRCustomerWarning && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Please add at least one purchase first, or set
                  VITE_BYPASS_STOCK_CHECK=true in .env to allow sales without
                  stock.
                </p>
              </div>
            </div>
          ...
          <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Please add MR and Customer data first to create or import
                  sales.
                </p>
              </div>
            </div>

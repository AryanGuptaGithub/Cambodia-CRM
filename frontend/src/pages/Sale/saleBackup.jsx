...
const isMRSaleDoc = (sale) => {
  // Primary check: exact saleType field (what DB actually stores)
  if (sale.saleType === "MR Sale") return true;
  if (sale.saleType === "Normal Sale") return false;

  // Legacy fallback for older records that may have boolean flags
  if (sale.isMRSale === true) return true;
  if (sale.isMrSaleImport === true) return true;

  // Last resort: partial string match
  if (
  ...
  // Default: treat unknown/missing saleType as Normal Sale
  return false;
};
...
const Sales = () => {
  ...
  // ✅ STEP 1: Define all callbacks FIRST before any useEffect that uses them
  ...
  const processSalesData = useCallback((data) => {
    // ✅ Handle both array and object with summaries key
    const salesData = Array.isArray(data) ? data : data?.summaries || data?.data || [];
    ...
    console.log("✅ processSalesData count:", sortedData.length);
    console.log("✅ First record saleType:", sortedData[0]?.saleType);
    setSales(sortedData);
  }, []);

  // ✅ STEP 2: fetchSaleSummaries defined BEFORE useEffects that reference it
  const fetchSaleSummaries = useCallback(async () => {
    try {
      ...
      console.log("✅ /all response count:", data);
      console.log("✅ First saleType:", data.summaries?.[0]?.saleType);

      // ✅ Pass data.summaries directly — processSalesData handles it
  ...
  // ✅ STEP 3: ALL useEffects AFTER all callbacks are defined
  ...
  // ✅ FIXED: fetchSaleSummaries now defined before this useEffect
  useEffect(() => {
  ...
  // ✅ Rest of your callbacks (these don't need to be before useEffects)
    ...
    async (sale) => {
      ...
      if (confirmDelete.isConfirmed) {
        try {
          ...
          if (res.status === 200) {
            showToast("success", `Sale ${sale.invoiceNumber} deleted successfully`);
            fetchSaleSummaries();
          }
        } catch (error) {
          const errorMessage =
  ...
  const calculateProductTotals = useCallback((products) => {
    if (!products || !Array.isArray(products))
      return { totalAmount: 0, totalDiscount: 0, netAmount: 0, totalProfitLoss: 0 };
    return products.reduce(
      (acc, product) => {
    ...
    async (e) => {
      ...
      try {
        ...
        const updatedForm = {
          ...
          dueAmount: (totals.netAmount - parseFloat(form.paidAmount || 0)).toFixed(2),
        };
        const token = localStorage.getItem("token");
    ...
    data.forEach((sale) => {
      const products =
        sale.products && sale.products.length
          ? sale.products
          : [{
              productName: "—", salesQty: 0, bonusQty: 0,
              totalQty: 0, sellingPrice: 0, discount: 0,
              netSellingAmount: 0, lc: 0, profitLoss: 0,
            }];
      ...
      <div className="container">
        ...
        {sales.length > 0 && (
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            {/* Left — payment status tabs + sale type tabs */}

...
import { X, Upload, CheckCircle, Download, AlertTriangle, User, Package, ChevronDown, ChevronRight } from "lucide-react";
...
// ----- Helper to filter numeric input -----
...
// ==========================================
// DuplicateInvoicesModal Component
// ==========================================
const DuplicateInvoicesModal = ({ isOpen, onClose, duplicates, onSkip, onCancel }) => {
    ...
    <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50">
      <div className="bg-white w-full max-w-2xl p-6 rounded-xl shadow-lg">
        ...
        <p className="text-sm text-gray-600 mb-6">
          You can skip these duplicates (they will be removed from the import list) or cancel the import.
        </p>
...
// ==========================================
// StockValidationModal (unchanged, reused for MR stock issues)
// ==========================================
          ...
          {isBlocked ? (
            <>
              <p className="font-semibold text-red-800">
                ⛔ IMPORT BLOCKED: {summary.totalInsufficient || 0} products have insufficient stock. You must:
              </p>
            ...
            <>
              <p className="font-semibold text-yellow-800">
                ⚠️ Missing Products Found: {summary.missingProducts || 0} products are not in inventory. These products will:
              </p>
              <ol className="mt-2 text-sm text-yellow-700 list-decimal list-inside space-y-1">
                <li>Be created automatically during import</li>
                <li>Have zero initial stock (you'll need to add inventory later)</li>
...
const ImportSalesModal = ({
  ...
  const validateMRStockBeforeImport = useCallback(async (invoices) => {
    try {
      ...
      setImportMessage(`Checking MR hand stock for ${invoices.length} invoices...`);
  ...
  const handleSkipDuplicates = useCallback(() => {
    ...
    setShowDuplicateModal(false);
    setDuplicateInvoices([]);
    showToast("info", `Skipped ${duplicateInvoices.length} duplicate invoice(s). Remaining: ${filteredData.length}`);
  ...
  const handleImportData = useCallback(async () => {
    ...
    await handleProductImport(parsedData);
  }, [parsedData, importSaleType, checkDuplicateInvoices, validateMRsBeforeImport, validateMRStockBeforeImport, validateStockBeforeImport]);
